...
import { distributeWeights, ensureWeights, scaleWeights, setWeightAndRebalance, weightsAreValid } from "@/lib/field-weights";
...
export function ListingDialog({ apartment, draft, onClose, onSave }: {
  ...
  async function save() {
    ...
    if (!weightsAreValid(fields)) { setError("Verteile genau 100 % Wichtigkeit auf die Fragen."); setTab("edit"); return; }
  ...
  return <div className="modal-backdrop listing-backdrop" role="presentation" onMouseDown={onClose}>
    <section className="modal listing-modal" role="dialog" aria-modal="true" aria-labelledby="listing-dialog-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      {tab === "edit" ? <div className="listing-editor" role="tabpanel">
        ...
        <div className="listing-fields-heading"><div><h3>Formularfelder</h3><p>Bestimme, welche Angaben Bewerbende machen sollen.</p></div><button type="button" className="button secondary add-field" onClick={() => { setFields((current) => { const added = newField(); return setWeightAndRebalance([...current, added], added.id, Math.round(100 / (current.length + 1))); }); setError(""); }}><PlusIcon /> Feld hinzufügen</button></div>
        <div className="weight-summary"><div><strong>Wichtigkeit verteilen</strong><p>Alle Fragen ergeben zusammen 100 %. Änderst du einen Anteil, passen sich die anderen proportional an. Die Wichtigkeit wirkt auf die Sortierung, sobald eine Bewertungsregel festgelegt ist.</p></div><div className="weight-controls"><span className={totalWeight === 100 ? "weight-total valid" : "weight-total invalid"} aria-live="polite">{totalWeight} / 100 %</span><button type="button" className="button secondary" onClick={() => { setFields((current) => distributeWeights(current)); setError(""); }}>Gleichmässig verteilen</button></div></div>
        <div className="listing-fields">
          {fields.map((field, index) => <div className="listing-field-card" key={field.id}>
            <div className="listing-field-top"><span>FELD {index + 1}</span><button type="button" className="row-icon-button danger" disabled={fields.length === 1} onClick={() => { setFields((current) => scaleWeights(current.filter((item) => item.id !== field.id))); setError(""); }} aria-label={`Feld ${index + 1} entfernen`} title={fields.length === 1 ? "Mindestens ein Feld ist erforderlich" : undefined}><TrashIcon /></button></div>
