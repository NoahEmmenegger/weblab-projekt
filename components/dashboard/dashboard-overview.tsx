...
export function DashboardOverview() {
  ...
  return <section className="dashboard-content">
    ...
    {viewingListing && <div className="modal-backdrop" role="presentation" onMouseDown={() => setViewingListing(null)}><section className="modal applications-modal" role="dialog" aria-modal="true" aria-labelledby="applications-title" onMouseDown={(event) => event.stopPropagation()}><button className="close-button" onClick={() => setViewingListing(null)} aria-label="Schliessen">×</button><p className="eyebrow">BEWERBUNGEN</p><h2 id="applications-title">{viewingListing.title}</h2>{loadingApplications ? <p>Lade Bewerbungen …</p> : submittedApplications.length ? <div className="submitted-list">{submittedApplications.map((application) => <article key={application.id}><strong>Eingegangen am {new Intl.DateTimeFormat("de-CH", { dateStyle: "medium", timeStyle: "short" }).format(new Date(application.createdAt))}</strong><dl>{application.fields.map((field) => <div key={field.id}><dt>{field.label}</dt><dd>{typeof application.answers[field.id] === "boolean" ? application.answers[field.id] ? "Ja" : "Nein" : String(application.answers[field.id] || "–")}</dd></div>)}</dl></article>)}</div> : <p className="modal-copy">Für diese Ausschreibung sind noch keine Bewerbungen eingegangen.</p>}</section></div>}
  </section>;
}
